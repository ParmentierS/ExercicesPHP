...
function pagination($content,$limite=4,$page=1)
{
    $length = count($content);

    for($i=1;$i<$length;$i++)
    {
        if(($page-1)*$limite < $i && $i <= ($page)*$limite)
        {
            echo "<article>";
            echo "<h3>".$content[$i]["titre"]."</h3>";
            echo "<p class=\"date\">".$content[$i]["ladate"]."</p>";
            echo "<p>".$content[$i]["letexte"]."</p>";
            echo "</article>";
        ...
        if($page != 1)
        {
            echo "<a href=pagination.php?page=". ($page - 1) . ">Page précédente</a>";
        }
        echo "<a href=pagination.php?page=". ($page + 1) . ">Page suivante</a>";
        echo "</nav>";
...
<body>
<?php pagination($contenu,$limite,$page); ?>
</body>